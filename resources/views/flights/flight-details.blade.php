    ...
    <div class="max-w-screen-xl mx-auto p-5 sm:p-10 md:p-16">
        <div class="border-b mb-5 flex justify-between text-sm">
            <div class="text-indigo-600 flex items-center pb-2 pr-2 border-b-2 border-indigo-600 uppercase">
                ...
                <h2 class="text-lg font-bold">Flights Service</h2>
            </div>
            {{-- <div class="flex items-center">
                <label for="filter-options" class="mr-2 text-gray-700">Select Filter:</label>
                <select id="filter-options" class="bg-indigo-600 text-white px-4 py-2 rounded">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            <div class="bg-white rounded-lg shadow-lg overflow-hidden relative">
                @if ($flights->first_image)
                    <img src="{{ asset('flights-images/' . $flights->first_image) }}" alt="Package Image" style="width:100%; height: 230px; object-fit: cover;">
                @else
                ...
                <div class="absolute top-2 right-2 bg-blue-500 text-white font-medium px-2 py-1 text-sm rounded-md">
                    PHP {{ number_format($flights->flights_price) }}
                </div>                   
        
                <div class="p-5">
                    <h3 class="text-lg font-semibold">{{ $flights->flights_title }}</h3>
                    <p class="text-gray-700">
                        {{ Str::limit($flights->flights_description, 120) }}
                    </p>
                    <a href="{{route('view-flights-details',$flights->id)}}" class="bg-blue-500 text-white rounded px-3 py-1 text-sm inline-block">
                        View Details
                    </a>
