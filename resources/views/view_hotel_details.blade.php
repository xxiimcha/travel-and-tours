        ...
        <div class="flex flex-wrap md:flex-nowrap">
            ...
            <div class="w-full md:w-1/2 bg-white rounded-lg shadow-lg p-6 mb-6 md:mb-0">
                ...
                <div class="relative">
                        ...
                        @if ($data->first_image)
                        <img src="{{ asset('hotel-images/' . $data->first_image) }}" alt="Package Image" style="width:100%; height:100%; object-fit: cover;">
                    @else
                    ...
                    <div class="absolute top-2 right-2 bg-blue-500 text-white font-medium px-3 py-1 text-sm rounded-md">
                        PHP {{ number_format($data->hotel_price) }} 
                    </div>
                ...
                <h3 class="text-xl font-semibold text-gray-800 mb-2">Room Type</h3>
                <p class="text-gray-600 mb-4">
                {{ $data->room_type }}
            </p>
                <h3 class="text-xl font-semibold text-gray-800 mb-2">Hotel Policy</h3>
                <p class="text-gray-600 mb-4">
                    {!!  $data->hotel_policy !!}
                </p>
                <!-- Book Now Button -->
            ...
            <div class="w-full md:w-1/2 ml-0 md:ml-6">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    @foreach ($images as $index => $image)
                        <img src="{{ asset('hotel-images/' . $image) }}" class="w-full h-40 object-cover rounded-md cursor-pointer transition duration-300 transform hover:scale-105" alt="Gallery Image {{ $index + 1 }}" onclick="openModal({{ $index }})">
                    @endforeach
                </div>
            </div>
...
<script>
    ...
    function openModal(index) {
        ...
        setTimeout(() => {
            modalImage.src = "{{ asset('hotel-images/') }}/" + images[currentIndex];
