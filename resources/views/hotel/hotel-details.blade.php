    ...
    <div class="max-w-screen-xl mx-auto p-5 sm:p-10 md:p-16">
        <div class="border-b mb-5 flex flex-col md:flex-row justify-between text-sm">
            ...
            <div class="text-indigo-600 flex items-center pb-2 pr-2 border-b-2 border-indigo-600 uppercase mb-4 md:mb-0">
                ...
                <h2 class="text-lg font-bold">Hotel Reservation</h2>
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            <div class="bg-white rounded-lg shadow-lg overflow-hidden relative">
                @if ($hotel->first_image)
                    <img src="{{ asset('hotel-images/' . $hotel->first_image) }}" alt="Package Image" style="width:100%; height: 230px; object-fit: cover;">
                @else
                ...
                <div class="absolute top-2 right-2 bg-blue-500 text-white font-medium px-2 py-1 text-sm rounded-md">
                    PHP {{ number_format($hotel->hotel_price) }}
                </div>                   
        
                <div class="p-5">
                    <h3 class="text-lg font-semibold">{{ $hotel->hotel_name }}</h3>
                    <p class="text-gray-700">
                        {{ Str::limit($hotel->hotel_description, 120) }}
                    </p>
                    <a href="{{route('view-hotel-details',$hotel->id)}}" class="bg-blue-500 text-white rounded px-3 py-1 text-sm inline-block">
                        View Details
                    </a>
